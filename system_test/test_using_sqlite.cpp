...
BOOST_AUTO_TEST_CASE(test_using_sqlite)
{
	ventura::absolute_path const app_source = repository / ventura::relative_path("application/using_sqlite");
	ventura::absolute_path const tmp = ventura::temporary_directory(Si::throw_) / *ventura::path_segment::create("cdm_test_using_sqlite");
	...
	std::unique_ptr<std::ofstream> log_file = cdm::open_log(tmp / ventura::relative_path("test_using_sqlite.txt"));
...
#endif
		        SILICIUM_SYSTEM_LITERAL("using_sqlite")
#ifdef _MSC_VER
