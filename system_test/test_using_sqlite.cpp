...
BOOST_AUTO_TEST_CASE(test_using_sqlite)
{
	...
	ventura::absolute_path const tmp = cdm::get_temporary_root_for_testing() / "cdm_test_using_sqlite";
	...
	std::unique_ptr<std::ofstream> log_file = cdm::open_log(application_build_dir / "test_using_sqlite.txt");
	...
	}
	{
		...
		ventura::relative_path const relative =
		    cdm::make_default_path_of_executable(*ventura::path_segment::create("using_sqlite"), target);
