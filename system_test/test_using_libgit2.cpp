#define CDM_CONFIGURE_NAMESPACE a0038873
#include "../../cdm/application/using_libgit2/cdm.hpp"
#include "log.hpp"
...
BOOST_AUTO_TEST_CASE(test_using_libgit2)
{
	ventura::absolute_path const app_source = repository / "application/using_libgit2";
	ventura::absolute_path const tmp = cdm::get_temporary_root_for_testing() / "cdm_test_using_libgit2";
	ventura::absolute_path const module_temporaries = tmp / "module_temporaries";
	ventura::absolute_path const application_build_dir = tmp / "using_libgit2";
	ventura::recreate_directories(module_temporaries, Si::throw_);
	ventura::recreate_directories(application_build_dir, Si::throw_);
	std::unique_ptr<std::ofstream> log_file = cdm::open_log(application_build_dir / "test_using_libgit2.txt");
	...
	}
	{
		...
		ventura::relative_path const relative =
		    cdm::make_default_path_of_executable(*ventura::path_segment::create("using_libgit2"), target);
