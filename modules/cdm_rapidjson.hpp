...
namespace cdm
{
	...
	inline rapidjson_paths install_rapidjson(ventura::absolute_path const &original_source, ventura::absolute_path const &temporarily_writable,
	                                         ventura::absolute_path const &install_root, Si::Sink<char, Si::success>::interface &output)
	{
		...
		if (!ventura::file_exists(in_cache, Si::throw_))
		{
			...
			ventura::copy_recursively(original_source / ventura::relative_path("include/rapidjson"), construction / ventura::relative_path("rapidjson"),
			                          &output, Si::throw_);
