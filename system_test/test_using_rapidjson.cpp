...
{
	ventura::absolute_path const app_source = repository / "application/using_rapidjson";
	ventura::absolute_path const tmp = cdm::get_temporary_root_for_testing() / "cdm_test_using_rapidjson";
	...
	}
	{
		...
		ventura::relative_path const relative =
		    cdm::make_default_path_of_executable(*ventura::path_segment::create("using_rapidjson"), target);
