...
namespace
{
	...
	void test_gtest_build_type(bool is_debug)
	{
		cdm::configuration target = cdm::approximate_configuration_of_this_binary();
		target.is_debug = is_debug;

		ventura::absolute_path const app_source = repository / "application/using_gtest";
		ventura::absolute_path const tmp = cdm::get_temporary_root_for_testing() / "cdm_test_using_gtest";
		ventura::absolute_path const module_temporaries = tmp / "module_temporaries";
		ventura::absolute_path const application_build_dir = tmp / "using_gtest";
		ventura::recreate_directories(module_temporaries, Si::throw_);
		ventura::recreate_directories(application_build_dir, Si::throw_);
		std::unique_ptr<std::ofstream> log_file = cdm::open_log(application_build_dir / "test_using_gtest.txt");
		...
		}
		{
			...
			ventura::relative_path const relative =
			    cdm::make_default_path_of_executable(*ventura::path_segment::create("using_gtest"), target);
			BOOST_REQUIRE_EQUAL(
...
BOOST_AUTO_TEST_CASE(test_using_gtest_debug)
{
	test_gtest_build_type(true);
}

BOOST_AUTO_TEST_CASE(test_using_gtest_release)
{
	test_gtest_build_type(false);
}
