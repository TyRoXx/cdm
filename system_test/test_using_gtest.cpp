...
{
	ventura::absolute_path const app_source = repository / ventura::relative_path("application/using_gtest");
	ventura::absolute_path const tmp = ventura::temporary_directory(Si::throw_) / *ventura::path_segment::create("cdm_test_using_gtest");
	...
	std::unique_ptr<std::ofstream> log_file = cdm::open_log(tmp / ventura::relative_path("test_using_gtest.txt"));
